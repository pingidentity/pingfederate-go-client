...
type KerberosRealm struct {
	// The persistent, unique ID for the Kerberos Realm. It can be any combination of [a-zA-Z0-9._-]. This property is system-assigned if not specified.
	...
	// For GET requests, this field contains the encrypted Domain/Realm password, if one exists. For POST and PUT requests, if you wish to reuse the existing password, this field should be passed back unchanged. Secret Reference may be provided in this field with format 'OBF:MGR:{secretManagerId}:{secretId}'. Only applicable when 'connectionType' is \"DIRECT\" or \"LOCAL_VALIDATION\".
