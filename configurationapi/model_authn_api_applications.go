// Copyright © 2026 Ping Identity Corporation

/*
Administrative API Documentation

The PingFederate Administrative API is a REST-based interface that provides a programmatic way to make configuration changes to PingFederate as an alternative to using the administrative console.<br/><br/>Expand the resources below to display implementation details on that resource such as the available endpoints, the parameter and response models for the operation, and the model structure of the resources themselves. Each resource operation comes with the ability to interact with the API. You are prompted for proper administration credentials when you try to perform an API operation.

API version: 13.0.0.6
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package configurationapi

import (
	"encoding/json"
)

// checks if the AuthnApiApplications type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &AuthnApiApplications{}

// AuthnApiApplications A collection of Authentication API Application items.
type AuthnApiApplications struct {
	// The actual list of Authentication API Applications.
	Items []AuthnApiApplication `json:"items,omitempty" tfsdk:"items"`
}

// NewAuthnApiApplications instantiates a new AuthnApiApplications object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewAuthnApiApplications() *AuthnApiApplications {
	this := AuthnApiApplications{}
	return &this
}

// NewAuthnApiApplicationsWithDefaults instantiates a new AuthnApiApplications object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewAuthnApiApplicationsWithDefaults() *AuthnApiApplications {
	this := AuthnApiApplications{}
	return &this
}

// GetItems returns the Items field value if set, zero value otherwise.
func (o *AuthnApiApplications) GetItems() []AuthnApiApplication {
	if o == nil || IsNil(o.Items) {
		var ret []AuthnApiApplication
		return ret
	}
	return o.Items
}

// GetItemsOk returns a tuple with the Items field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *AuthnApiApplications) GetItemsOk() ([]AuthnApiApplication, bool) {
	if o == nil || IsNil(o.Items) {
		return nil, false
	}
	return o.Items, true
}

// HasItems returns a boolean if a field has been set.
func (o *AuthnApiApplications) HasItems() bool {
	if o != nil && !IsNil(o.Items) {
		return true
	}

	return false
}

// SetItems gets a reference to the given []AuthnApiApplication and assigns it to the Items field.
func (o *AuthnApiApplications) SetItems(v []AuthnApiApplication) {
	o.Items = v
}

func (o AuthnApiApplications) MarshalJSON() ([]byte, error) {
	toSerialize, err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o AuthnApiApplications) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if !IsNil(o.Items) {
		toSerialize["items"] = o.Items
	}
	return toSerialize, nil
}

type NullableAuthnApiApplications struct {
	value *AuthnApiApplications
	isSet bool
}

func (v NullableAuthnApiApplications) Get() *AuthnApiApplications {
	return v.value
}

func (v *NullableAuthnApiApplications) Set(val *AuthnApiApplications) {
	v.value = val
	v.isSet = true
}

func (v NullableAuthnApiApplications) IsSet() bool {
	return v.isSet
}

func (v *NullableAuthnApiApplications) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableAuthnApiApplications(val *AuthnApiApplications) *NullableAuthnApiApplications {
	return &NullableAuthnApiApplications{value: val, isSet: true}
}

func (v NullableAuthnApiApplications) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableAuthnApiApplications) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}
