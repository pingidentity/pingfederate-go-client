...
func main() {
	...
	// token is available), then builds the generated admin API client in one call. The returned
	// client's HTTP client already injects the resolved token on every request, so it can be
	// used directly with any context.
